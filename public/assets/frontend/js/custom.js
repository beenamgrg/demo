...
$(document).ready(function () {
  $(window).on("scroll", function () {
    const triggerDiv = $(".testimony-box");
    const isInView =
      triggerDiv.offset().top <= $(window).scrollTop() + $(window).height();

    if (isInView) {
      triggerDiv.addClass("testimony-box-animation");
      $(window).off("scroll");
    }

    const triggerDiv2 = $(".service-bubble-animation");
    const isInView2 =
      triggerDiv.offset().top <= $(window).scrollTop() + $(window).height();

    if (isInView2) {
      triggerDiv2.addClass("service-bubble-animation");
      $(window).off("scroll");
    }
  });
});
